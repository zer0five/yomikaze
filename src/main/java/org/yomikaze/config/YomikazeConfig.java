...
import org.yomikaze.config.properties.YomikazeProperties;
import org.yomikaze.task.WakeMyDyno;
...
public class YomikazeConfig {
    ...
    public PasswordEncoder passwordEncoder(YomikazeProperties properties) {
        YomikazeProperties.Security.Password.Encoder encoder = properties.getSecurity().getPassword().getEncoder();
        YomikazeProperties.Security.Password.Encoder.PasswordEncoderType encoderType = encoder.getType();
        switch (encoderType) {
            case ARGON2:
                YomikazeProperties.Security.Password.Encoder.Argon2 argon2 = encoder.getArgon2();
                return argon2.get();
            case BCRYPT:
                YomikazeProperties.Security.Password.Encoder.BCrypt bcrypt = encoder.getBcrypt();
                return bcrypt.get();
            default:
